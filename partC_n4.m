...
for Particle=1:PopulationSize
    PBestFitnesses(Particle) = (a4*sum(Positions_X(Particle,:).^4))+(a3*sum(Positions_X(Particle,:).^3))+(a2*sum(Positions_X(Particle,:).^2))+(a1*sum(Positions_X(Particle,:) ))+a0;
...
for Iteration = 1:MaxIterations
    ...
    fprintf('a4: %d a3: %d a2: %d a1: %d a0: %d  Iteration: %d   Bset Fitness: %e ',a4,a3, a2,  a1,  a0,Iteration, LBestFitness);
    g=sprintf('%d ', CurrentPosition);
    fprintf(' Roots X1 X2 X3 X4: %s ',g)
